...
export function SectionHeader({
  ...
  const colorClass =
    labelColor === 'green' ? 'text-accent-green' : 'text-accent-blue'
  const alignClass = align === 'center' ? 'text-center' : ''

  return (
    <div className={alignClass}>
      <p
        className={`text-[13px] tracking-[3px] mb-1 font-semibold uppercase ${colorClass}`}
      >
        {label}
      </p>
      <h2 className="text-2xl font-bold tracking-tight text-text">{title}</h2>
      {subtitle && (
        <p className="text-[15px] text-text-muted mt-2">{subtitle}</p>
      )}
      {children}
    </div>
  )
